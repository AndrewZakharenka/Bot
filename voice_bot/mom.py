...
Принимает транскрипцию встречи (с разметкой собеседников или без),
возвращает структурированные итоги.
"""
from voice_bot import brain

_MOM_PROMPT = """Ты — ассистент для встреч. Тебе дана транскрипция встречи.
Реплики могут быть размечены по участникам в формате **Имя:** текст.
...
def generate(transcript: str) -> str:
    ...
    transcript — распознанный текст встречи (может содержать **Имя:** разметку)
